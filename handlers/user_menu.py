...
async def instruction_key(msg: Message, state: FSMContext):
    ...
    if active_key is None:
        ...
        await msg.answer(text="📌 Выберите устройство, на которое планируете установить ключ:",
                         reply_markup=await choosing_a_device())
        await msg.answer(text=f"⚠️<b>Не делитесь ключом.</b> При использовании на устройствах сверх лимита подписки "
...
async def help_section(msg: Message):
    await msg.answer(text=
        "📌 Если у вас возникнут какие-либо трудности или неполадки при использовании программы —"
        "обязательно обращайтесь в нашу службу поддержки.\n\n"
        "Мы оперативно разберёмся в вашей ситуации и поможем найти решение 👍\n\n"
        "Не откладывайте — пишите сразу, и все вопросы будут решены в кратчайшие сроки 💬✨\n\n"
        f"Связаться с нами можно здесь:\n"
        f"[Аккаунт поддержки](https://t.me/@Rust_1_1_1)",
                     parse_mode='Markdown'
    )
